...
// Render instructional shift
export const renderInstructionalShift = async (pdf: jsPDF, form: FormData, currentY: number): Promise<number> => {
  ...
  autoTable(pdf, {
    startY: currentY,
    head: [[`Instructional Shift ${form.formNumber}`]],
    body: [
      ...
      ['Feedback', form.content?.feedback || 'N/A']
    ],
    theme: 'grid',
...
// Render shift evaluation
export const renderShiftEvaluation = async (pdf: jsPDF, form: FormData, currentY: number): Promise<number> => {
  if (currentY > pdf.internal.pageSize.getHeight() - 60) {
    pdf.addPage();
    currentY = 20;
  }
  ...
  autoTable(pdf, {
    startY: currentY,
    head: [[`Shift Evaluation ${form.formNumber}`]],
    body: [
...
// Render case summary
export const renderCaseSummary = async (pdf: jsPDF, form: FormData, currentY: number): Promise<number> => {
  if (currentY > pdf.internal.pageSize.getHeight() - 60) {
    pdf.addPage();
    currentY = 20;
  }
  ...
  autoTable(pdf, {
    startY: currentY,
    head: [[`Case Summary ${form.formNumber}`]],
    body: [
